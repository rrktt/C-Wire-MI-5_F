...
hvaComp(){
    ...
    if [ $# -ne 4 ] ; then 
        ...
        awk -F';' '$3 != "-" && $4 == "-" {print $3" "$7" "$8}' input/data.dat > tmp/tmp.txt #take the important information from hva stations
...
hvbComp(){
    ...
    if [ $# -ne 4 ] ; then
        ...
        awk -F';' '$2 != "-" && $3 == "-" {print $2" "$7" "$8}' input/data.dat > tmp/tmp.txt #take the important information from hvb stations
...
lvComp(){
    ...
    if [ $# -ne 4 ] ; then
        ...
        awk -F';' '$6 == "-" && $4 != "-" && $2 == "-" {print $4" "$7" "$8}' input/data.dat > tmp/tmp.txt #take the important information from lv stations
...
lvIndiv(){
    ...
    if [ $# -ne 4 ] ; then
        ...
        awk -F';' '$5 == "-" && $4 != "-" && $2 == "-" {print $4" "$7" "$8}' input/data.dat > tmp/tmp.txt #take the important information from lv stations
...
if [ $rep1 -eq 0 ] ; then #if arguments are correct
    ...
    if [ ! -e graphs/ ] ; then #test the presence of graphs directory
        ...
        rm -rf graphs/* #erase file in the directory
    ...
    echo "time to sort the output file and create graph if it could : $deltaT s"
